...
│   │   ├── BotConfig.ts          # Singleton: ForceSub & Testimonial channel configuration
...
│   │   ├── admin/                # Master admin panel (OTP settings, ForceSub, Testimonial)
...
| **`smsbower`** | `product_otp`, `otp_*` callbacks | All Users | Interactive virtual phone number rental for SMS verification, real-time OTP status polling (10s interval, 10 min max), refund on cancellation/timeout. |
| **`admin`** | `/admin`, `/stats`, `/statistik`, `adm_*` callbacks | Admin (`ADMIN_ID`) | Master control for Bot Analytics/Stats, OTP toggle, country & service whitelists, price markup settings, Force-Subscription channel setup, and Testimonial channel configuration. |
| **`digiadmin`**| `dga_*` callbacks | Admin (`ADMIN_ID`) | Digital store manager: Add/edit/delete products, toggle active status, bulk stock injection (`dga_stock`), and revenue/sales metrics. |
...
- **Key Fields**: `name`, `category` (indexed), `description`, `price` (IDR), `isActive` (Boolean). Compound index on `{ category: 1, isActive: 1 }`.
...
- **Key Fields**: `orderId` (Unique), `userId`, `productId`, `productName`, `quantity`, `price`, `itemContent`, `createdAt`.
...
- **Purpose**: Runtime configuration for Force Subscription and Testimonial broadcasting channels.
- **Key Fields**: `forceSubEnabled`, `forceSubChannel`, `forceSubLink`, `forceSubName`, `testimonialEnabled`, `testimonialChannel`, `testimonialLink`.
...
| `ADMIN_ID` | **Yes** | Numeric Telegram ID of the bot administrator | `123456789` |
