...
- 📱 **WhatsApp Bot Engine (Baileys)**: Multi-platform support (`src/whatsapp/`) enabling simultaneous WhatsApp bot operation using WebSocket multi-device protocol, terminal QR or 8-digit pairing code, conversational state machine, automated digital purchases, QRIS topups, and live SMS OTP verification.
