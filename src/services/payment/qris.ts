...
export class QrisGenerator {
  ...
  public async getDynamicPayload(amount: number): Promise<string> {
    ...
    return createDynamicPayload(staticPayload.trim(), amount);
  }

  public async generate(amount: number): Promise<string> {
    const dynamicPayload = await this.getDynamicPayload(amount);
