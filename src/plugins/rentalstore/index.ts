...
export interface RentalStoreDependencies {
  ...
  provision(input: { ownerTelegramId: string; planId: string; botToken: string }): Promise<ProvisionedRental>;
  createInvoice(rentalId: string, actorTelegramId: string, planId: string): Promise<RentalInvoice>;
  checkPayment(providerReference: string, actorTelegramId: string): Promise<RentalPaymentResult>;
...
function rentalLine(rental: OwnedRentalSummary): string {
  const expiry = rental.status === "pending" ? "menunggu pembayaran" : `${formatDate(rental.expiresAt)} WIB`;
...
export function createRentalStorePlugin(overrides: Partial<RentalStoreDependencies> = {}): Plugin {
  ...
  function cleanup(now: number): void {
    ...
    if (tokenAttempts.size > 10_000) {
      for (const [owner, state] of tokenAttempts) if (state.resetsAt <= now) tokenAttempts.delete(owner);
    }
  ...
  async function rejectNonPrivate(ctx: Context): Promise<boolean> {
    ...
    if (ctx.callbackQuery) {
      await ctx.answerCallbackQuery({ text: "Buka menu sewa melalui chat pribadi bot.", show_alert: true }).catch(() => {});
    } else {
      await ctx.reply("🔒 Sewa bot hanya dapat diproses melalui chat pribadi.").catch(() => {});
    }
    return true;
  }
  ...
  async function showShop(ctx: Context): Promise<void> {
    ...
    try {
      ...
      if (rental) {
        ...
        if (url && rental.status !== "pending") keyboard.url("🤖 Buka Bot Saya", url).row();
        for (const plan of plans) {
          keyboard.text(planButton(plan), `rs_pay_${rental.rentalId}_${plan.id}`).row();
        }
        await ctx.reply(
          `🤖 Rental Saya\n\n${rentalLine(rental)}\n\n` +
          (plans.length ? "Pilih paket untuk aktivasi atau memperpanjang masa sewa." : "Belum ada paket rental aktif."),
          { reply_markup: keyboard },
        );
        return;
      }
      for (const plan of plans) keyboard.text(planButton(plan), `rs_new_${plan.id}`).row();
      await ctx.reply(
        "🤖 Sewa Bot Otomatis\n\n" +
        "1. Pilih paket.\n" +
      ...
      await ctx.reply("⚠️ Sewa bot otomatis belum tersedia. Pastikan layanan rental dan pembayaran platform sudah dikonfigurasi.");
    }
  }

  async function sendInvoice(ctx: Context, invoice: RentalInvoice, botUsername: string): Promise<void> {
    const keyboard = new InlineKeyboard()
    ...
    await ctx.replyWithPhoto(new InputFile(invoice.qris.buffer, "rental-qris.png"), {
      caption:
  ...
  async function selectNewPlan(ctx: Context, planId: string): Promise<void> {
    ...
    try {
      ...
      if (rental) {
        await ctx.reply("Kamu sudah memiliki rental. Buka /sewa untuk aktivasi, status, atau perpanjangan.");
        return;
      }
      const plan = plans.find(item => item.id === planId);
      if (!plan) {
        await ctx.reply("Paket sudah tidak tersedia. Buka /sewa untuk memuat daftar terbaru.");
        return;
      }
      const now = dependencies.now();
      cleanup(now);
      setRentalTokenInput(ownerTelegramId, { planId: plan.id, expiresAt: now + TOKEN_INPUT_TTL_MS });
      await ctx.reply(
      ...
      await ctx.reply("Paket belum dapat diproses. Buka /sewa dan coba kembali.");
    }
  }

  async function createExistingInvoice(ctx: Context, rentalId: string, planId: string): Promise<void> {
    if (await rejectNonPrivate(ctx) || !ctx.from) return;
    await ctx.answerCallbackQuery().catch(() => {});
    const ownerTelegramId = String(ctx.from.id);
    try {
      ...
      const invoice = await dependencies.createInvoice(rentalId, ownerTelegramId, planId);
      if (invoice.payment.planId !== planId) {
        await ctx.reply("Masih ada invoice sebelumnya yang aktif. Selesaikan invoice itu atau tunggu hingga kedaluwarsa.");
      }
      await sendInvoice(ctx, invoice, rental.botUsername);
    } catch {
      console.warn(`[Platform] Rental invoice creation failed for rental ${rentalId}.`);
      await ctx.reply("Invoice belum dapat dibuat. Periksa konfigurasi pembayaran atau coba kembali nanti.");
    }
  }

  async function checkInvoice(ctx: Context, providerReference: string): Promise<void> {
    if (await rejectNonPrivate(ctx) || !ctx.from) return;
    await ctx.answerCallbackQuery().catch(() => {});
    const ownerTelegramId = String(ctx.from.id);
    const now = dependencies.now();
    ...
    if (lastCheck > 0 && now - lastCheck < PAYMENT_CHECK_COOLDOWN_MS) {
      await ctx.reply("⏳ Pemeriksaan baru saja dilakukan. Tunggu sekitar 10 detik.");
      return;
    }
    lastPaymentChecks.set(cooldownKey, now);
    try {
      const result = await dependencies.checkPayment(providerReference, ownerTelegramId);
      if (result.status === "paid") {
        let runtimeReady = true;
        try { await dependencies.startRental(result.rentalId); }
        catch {
          runtimeReady = false;
          console.warn(`[Platform] Paid rental ${result.rentalId} is waiting for runtime retry.`);
        }
      ...
      await ctx.reply(result.status === "expired"
      ...
      await ctx.reply("Pembayaran belum dapat diperiksa. Coba lagi nanti; pemeriksaan otomatis tetap berjalan.");
    }
  }

  async function receiveToken(ctx: Context, next: () => Promise<void>): Promise<void> {
    ...
    if (!wasRentalTokenMessageDeleted(ctx)) {
      try {
        ...
        clearRentalTokenInput(ownerTelegramId);
        await ctx.reply("⚠️ Pesan token tidak berhasil dihapus, jadi token tidak diproses. Hapus dan rotasi token di @BotFather, lalu mulai lagi melalui /sewa.");
        return;
      }
    }
    clearRentalTokenInput(ownerTelegramId);
    ...
    if (state.expiresAt <= now) {
      await ctx.reply("Waktu pengiriman token sudah habis. Mulai kembali melalui /sewa.");
      return;
    }
    if (!consumeTokenAttempt(ownerTelegramId, now)) {
      await ctx.reply("Terlalu banyak percobaan verifikasi token. Tunggu hingga satu jam lalu coba lagi.");
      return;
    }
    const botToken = rawText.trim();
    if (botToken.length > 256 || !/^\d+:[A-Za-z0-9_-]{20,}$/.test(botToken)) {
      await ctx.reply("Format token tidak valid. Token tidak disimpan. Mulai kembali melalui /sewa.");
      return;
    }
    let created: ProvisionedRental | undefined;
    try {
      created = await dependencies.provision({ ownerTelegramId, planId: state.planId, botToken });
      const invoice = await dependencies.createInvoice(created.rentalId, ownerTelegramId, created.planId);
      await ctx.reply(`✅ Bot @${created.botUsername} berhasil diverifikasi. Bot tetap offline sampai pembayaran terkonfirmasi.`);
  ...
  return {
    ...
    register(bot: Bot<Context>): void {
      ...
      bot.callbackQuery(/^rs_new_([a-f0-9]{24})$/, ctx => selectNewPlan(ctx, ctx.match[1]!));
      bot.callbackQuery(/^rs_pay_([a-f0-9]{24})_([a-f0-9]{24})$/, ctx => createExistingInvoice(ctx, ctx.match[1]!, ctx.match[2]!));
      bot.callbackQuery(/^rs_chk_([A-Za-z0-9_-]{1,96})$/, ctx => checkInvoice(ctx, ctx.match[1]!));
