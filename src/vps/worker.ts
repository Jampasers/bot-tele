...
export async function advanceVpsOrder(order: IVpsOrder, deps: VpsStepDependencies): Promise<void> {
  ...
  if (order.stage === "monitoring") {
    ...
    if (successes >= 3) { await stage("ready", { evidence: "RDP terjangkau pada 3 pemeriksaan. Login Windows belum diverifikasi; silakan uji melalui Remote Desktop." }); deps.clearToken(); }
