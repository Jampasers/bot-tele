...
Bot platform juga memuat alur self-service melalui `/sewa` dan **Catalog → 🤖 Sewa Bot**. Pengguna memilih paket aktif dan mengirim token BotFather baru melalui chat pribadi. Handler wajib menghapus pesan sebelum memverifikasi atau menyimpan token. Rental dibuat dalam status `pending`, lalu aktivasi awal memotong saldo user pada main bot secara atomik dan idempotent. Jika saldo kurang, sistem dapat menerbitkan invoice QRIS platform sebagai cadangan. Instance rental ditahan offline sampai pembayaran berhasil, kemudian masa sewa diaktifkan dan instance dimulai. Alur publik membatasi satu rental yang belum `terminated` per owner Telegram; provisioning admin tetap dapat membuat lebih dari satu rental.
...
`BOT_TOKEN`, `ADMIN_ID`, MongoDB dan environment fitur platform existing tetap dipakai. `GOPAY_MERCHANT_ID`, QRIS platform serta `GOJEK_EMAIL`/`GOBIZ_EMAIL` dan password terkait dipakai **hanya oleh payment platform**. Konfigurasi tersebut tidak diperlukan untuk membuka katalog atau mengaktifkan rental awal jika saldo main bot mencukupi. Renter tanpa konfigurasi merchant tidak mendapat fallback ke payment platform. Invoice platform ditolak jika konfigurasi login merchant belum tersedia.
